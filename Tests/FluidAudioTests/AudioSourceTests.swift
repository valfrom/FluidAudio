...
final class AudioSourceTests: XCTestCase {
    ...
    func testConcurrentAudioSources() async throws {
        ...
        do {
            ...
            XCTAssertFalse(asrManager.isAvailable, "ASR should not be available if initialization failed")
    ...
    func testMultipleConcurrentTranscriptions() async throws {
        ...
        do {
            ...
            let results = try await withThrowingTaskGroup(of: (AudioSource, ASRResult).self) { group in
                for _ in 0..<2 {
                    group.addTask {
