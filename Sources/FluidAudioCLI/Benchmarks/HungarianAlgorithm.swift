...
public struct HungarianAlgorithm {
    ...
    private static func findAugmentingPath(
        matrix: [[Float]], row: Int,
        ...
        for col in 0..<matrix[row].count {
            if matrix[row][col] == 0 && !colCovered[col] {
                ...
                if revAssignments[col] == -1
                    || findAugmentingPath(
                        matrix: matrix, row: revAssignments[col],
                        assignments: &assignments,
                        revAssignments: &revAssignments,
                        rowCovered: &rowCovered,
                        colCovered: &colCovered)
    ...
    private static func coverColumnsOptimally(
        matrix: [[Float]],
        rowCovered: inout [Bool],
        colCovered: inout [Bool]
    ) {
...
extension HungarianAlgorithm {
    ...
    public static func createSpeakerMapping(
        assignments: [Int],
        predSpeakers: [String],
        gtSpeakers: [String]
    ) -> [String: String] {
