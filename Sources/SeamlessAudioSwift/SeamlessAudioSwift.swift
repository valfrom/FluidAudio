...
// MARK: - Main Speaker Diarization Manager
...
public final class SpeakerDiarizationManager {
    ...
    private func getModelsDirectory() -> URL {
        ...
        if let customDirectory = config.modelCacheDirectory {
            ...
            directory = appSupport.appendingPathComponent("SpeakerKitModels", isDirectory: true)
        }
    ...
    public func compareSpeakers(audio1: [Float], audio2: [Float]) async throws -> Float {
        ...
        guard let emb1 = embedding1, let emb2 = embedding2 else {
            throw SpeakerDiarizationError.embeddingExtractionFailed
        }
...
public enum SpeakerDiarizationError: Error, LocalizedError {
    case notInitialized
    ...
    public var errorDescription: String? {
        switch self {
            case .notInitialized:
                return "Speaker diarization system not initialized. Call initialize() first."
            case .modelDownloadFailed:
