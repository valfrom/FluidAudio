...
func sherpaOnnxOfflineSpeakerDiarizationConfig(
    ...
    minDurationOn: Float = 0.05,
    minDurationOff: Float = 0.03
...
class SherpaOnnxOfflineSpeakerDiarizationWrapper {
    ...
    print("[🧪 Diarizer] Processing sample count: \(samples.count)")
    print("[🧪 Diarizer] First 10 samples:", samples.prefix(10))

    // 1) Make sure impl is non-nil
    guard let impl = self.impl else {
        print("[❌ Diarizer] impl is nil. Initialization may have failed.")
        return []
    }
    ...
    guard let result = SherpaOnnxOfflineSpeakerDiarizationProcess(
        ...
        print("[❌ Diarizer] Process returned nil")
        return []
    }

    // 3) Check how many segments we got
    let numSegments = Int(
        SherpaOnnxOfflineSpeakerDiarizationResultGetNumSegments(result)
    )
    print("[🧮 Diarizer] numSegments = \(numSegments)")

    guard numSegments > 0 else {
        print("[⚠️ Diarizer] No segments detected in this audio.")
